...
module Admin
  ...
  def destroy
    user = User.find(params[:id])
    if user.isadmin?
      redirect_to admin_users_path, alert: 'Nie można usunąć konta administratora.'
    else
      user.destroy
    ...
    def user_params
      params.require(:user).permit(:name, :login, :password, :password_confirmation, :isadmin, room_ids: [])
    end    
